...
aws_cmd s3api head-bucket --bucket "$BUCKET_NAME" >/dev/null 2>&1 || fail "Existing S3 bucket is unavailable: $BUCKET_NAME"
...
[[ -n "$OAC_ID" && "$OAC_ID" != "None" ]] || fail "Existing CloudFront OAC was not found: $OAC_NAME"
...
# Create a temporary authorized copy. The tracked V1.1 file remains unchanged.
sed "s|:user/proud-pops-demo-deployer|:user/${EXPECTED_ARN##*/}|g" "$BASE_SCRIPT" > "$TEMP_SCRIPT"
chmod +x "$TEMP_SCRIPT"
bash -n "$TEMP_SCRIPT"
grep -Fq "$EXPECTED_ARN" "$TEMP_SCRIPT" || fail "Could not authorize expected IAM identity in temporary script."

log "Starting deployment through the corrected V1.1 engine..."
PROUDPOPS_DEMO_AWS_PROFILE="$AWS_PROFILE_NAME" \
PROUDPOPS_DEMO_AWS_ACCOUNT="$EXPECTED_ACCOUNT" \
AWS_REGION="$REGION" \
PROUDPOPS_GITHUB_BRANCH="$GITHUB_BRANCH" \
PROUDPOPS_DEMO_STATE_DIR="$STATE_DIR" \
"$TEMP_SCRIPT"
