...
COMMIT_MESSAGE="Apply guided service selector design V1.1"
...
log "=== Finish Proud Pops Guided Service Selector V1.1 ==="
...
# Recovery mode accepts only the two files generated by the failed V1 run.
CHANGED_PATHS="$WORK_DIR/changed-paths.txt"
git status --porcelain | sed -E 's/^.. //' | sort -u > "$CHANGED_PATHS"
[[ -s "$CHANGED_PATHS" ]] || fail "No pending Guided Selector changes were found."
while IFS= read -r changed; do
  [[ -n "$changed" ]] || continue
  case "$changed" in
    public-site/services.html|public-site/assets/css/services.css|public-site/assets/css/site.css)
      log "EXPECTED PENDING CHANGE: $changed"
      ;;
    *)
      fail "Unexpected modified file: $changed"
      ;;
  esac
done < "$CHANGED_PATHS"
...
TAB_COUNT="$(grep -o 'data-service-tab=' "$SERVICES_HTML" | wc -l | tr -d ' ')"
PANEL_COUNT="$(grep -o 'data-service-panel=' "$SERVICES_HTML" | wc -l | tr -d ' ')"
[[ "$TAB_COUNT" -eq 5 ]] || fail "Expected five service tabs; found $TAB_COUNT."
[[ "$PANEL_COUNT" -eq 5 ]] || fail "Expected five service panels; found $PANEL_COUNT."
